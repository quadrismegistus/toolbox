...
def default_meter(default='meter_ryan'):
	md=meters[default].config
	md['constraints'].sort()
	return md
	#return {'maxS':2, 'maxW':2, 'splitheavies':0, 'constraints':[]}
...
class WebText(p.Text):
	...
	def iparse_rows(self,meter=None,all_parses=True,viols=True):
		...
		for line_i,line in enumerate(self.iparse(meter=meter)):
			dx={'#':line_i+1, line_hdr:unicode(line)}
			...
			for k,v in parse2dict(bp,meter,line_id=line_i+1).items(): dx[k]=v

			dx['all_parses'] = parses = []
			if all_parses:
				for api,parse in enumerate(ap):
					dx2={'#':api+1, line_hdr:unicode(line), '# parse':''}
					for k,v in parse2dict(parse,meter,line_id=str(line_i+1)+'_'+str(api+1)).items(): dx2[k]=v
					parses+=[dx2]
			yield dx
